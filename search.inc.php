<form id="search" action="<?php echo createURL('search'); ?>" method="post">
    ...
    <tr>
        ...
        if ($logged_on || isset($range)) {
        ?>
        <td><?php echo T_('Search' /* Search ... for */); ?></td>
        <td>
            <select name="range">
                <?php
                if (isset($range) && !in_array($range, array($currentUsername, 'all', 'watchlist'))) {
                ?>
                <option value="<?php echo $range ?>"><?php echo T_("this user's bookmarks"); ?></option>
                <?php
                }
                if ($logged_on) {
                ?>
